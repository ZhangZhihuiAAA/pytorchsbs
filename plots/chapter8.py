...
from plots.chapterextra import add_arrow
from plots.replay import FeatureSpace
from plots.replay import build_feature_space
...
def counter_vs_clock(basic_corners=None, basic_colors=None, basic_letters=None, draw_arrows=True, binary=True):
    ...
    fig, axs = plt.subplots(1, 1 + draw_arrows, figsize=(3 * ( 1 + draw_arrows), 3))
    if not draw_arrows:
        axs = [axs]
    ...
    for is_clockwise in range(1 + draw_arrows):
        if draw_arrows:
            if binary:
                if is_clockwise:
                    axs[is_clockwise].text(-0.5, 0, 'Clockwise')
                    axs[is_clockwise].text(-0.2, -0.25, 'y=1')
                else:
                    axs[is_clockwise].text(-0.5, 0, ' Counter-\nClockwise')
                    axs[is_clockwise].text(-0.2, -0.25, 'y=0')
        ...
        for i in range(4):
            ...
            if not binary:
                targets = [2, 3] if is_clockwise else [1, 2]
            else:
                targets = []

            alpha = 0.3 if i in targets else 1.0
            axs[is_clockwise].scatter(*coords, c=color, s=400, alpha=alpha)
            ...
            line = axs[is_clockwise].plot(*arrow_coords.T, c=color, lw=0 if draw_arrows else 2, alpha=alpha, 
                                          linestyle='--' if (alpha < 1) and (not draw_arrows) else '-')[0]
            
            if draw_arrows:
                add_arrow(line, lw=3, alpha=alpha)

            axs[is_clockwise].set_xlabel(r'$x_0$')
            axs[is_clockwise].set_ylabel(r'$x_1$', rotation=0)
            limits = np.stack([corners.min(axis=0), corners.max(axis=0)])
            limits = limits.mean(axis=0).reshape(2, 1) + 1.2 * np.array([[-factor, factor]])
            axs[is_clockwise].set_xlim(limits[0])
            axs[is_clockwise].set_ylim(limits[1])
            axs[is_clockwise].grid(False)
            axs[is_clockwise].text(*(coords - factor * np.array([0.05, 0.05])), letter, c='k' if i in targets else 'w', fontsize=12)
...
def plot_sequences(basic_corners=None, basic_colors=None, basic_letters=None, binary=True, target_len=0):
    ...
    for row in range(4):
        for col in range(2):
            for i in range(4):
                colors = np.array(basic_colors)[[(row + i) % 4 for i in range(4)]][slice(None, None, col * 2 - 1)]
                letters = np.array(basic_letters)[[(row + i) % 4 for i in range(4)]][slice(None, None, col * 2 - 1)]
                axs[row, col].set_xticks([])
                axs[row, col].set_yticks([])
                axs[row, col].set_xlim([-0.5, 4])
                axs[row, col].set_ylim([-1, 1])
                axs[row, col].grid(False)
                axs[row, col].scatter(i, 0, c=colors[i], s=600, alpha=0.3 if (i + target_len) >= 4 else 1.0)
                axs[row, col].text(i - 0.125, -0.2, letters[i], c='k' if (i + target_len) >= 4 else 'w', fontsize=14)
                if binary:
                    axs[row, col].text(4, -0.1, f'y={col}')
...
def plot_data(points, directions, n_rows=2, n_cols=5):
    ...
    for e, ax in enumerate(axs):
        ...
        for i in range(4):
            ...
            ax.scatter(*pred_corners.T, c='k', s=400)
            start = -1 if i == 3 else i
